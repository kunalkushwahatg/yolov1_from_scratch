...
for epoch in range(epochs):
    ...
    for batch in train_loader:
        ...
        # Forward pass
        output = model(img_data.to(device))

        # Compute the loss
        loss = criteriation(output, label.to(device))
    ...
    if epoch % print_every == 0:
        ...
        val_loss = eval.dataloader_loss(model,criteriation,val_laoder)
